...
export default function Sidebar() {
  const [activeTab, setActiveTab] = useState('trade'); // Default active tab
      ...
      icon: (
        ...
        >
          <path
            ...
            fill="currentColor"
            className={`transition-colors duration-200 ${
              activeTab === 'wallet' ? 'text-[#225FED]' : 'text-white group-hover:text-[#225FED]'
            }`}
          />
        </svg>
      )
          ...
          <div key={item.id} className="group cursor-pointer">
            <Tooltip>
              <TooltipTrigger className="flex items-center">
                <div 
                  className={`w-full mx-1 p-[16px] pr-[24px] ${
                    activeTab === item.id 
                      ? 'bg-black/12 rounded-l-[12px] border-r-4 border-[#225FED] mr-0'
                      : 'rounded-l-[12px] group-hover:bg-black/12 pr-[28px]'
                  }`}
