    ...
    <div className="w-full max-w-[1240px] mx-auto mt-10 rounded-xl overflow-hidden border border-blue-200">
      ...
      <div className="relative bg-gradient-to-r from-[#1F4293] via-[#8A38F5] to-[#26F6BA] px-6 pt-2 pb-0 rounded-t-xl">
        <div className="flex items-end space-x-2 relative">
          <div
            className={`absolute transition-all duration-300 ease-in-out bg-white rounded-t-xl z-0 ${
              isTradeTab ? "default-highlight" : "unrealized-highlight"
            }`}
            style={{
              height: "100%",
              width: "8rem",
              transform: `translateX(${tabs.indexOf(activeTab) * 8.5}rem)`,
            }}
          />
          {tabs.map((tab) => (
            <button
              ...
              className={`relative z-10 px-4 py-2 text-sm font-semibold transition-all duration-200 ${
                activeTab === tab
                  ? "text-[#1F4293]"
      ...
      <div className="bg-white">
        {isTradeLike &&
          transactions.map((tx, idx) => (
            <div
              key={tx.id}
              className={`grid grid-cols-8 gap-4 px-6 py-3 text-sm ${
        ...
        {activeTab === "Unrealized P&L" &&
          unrealized.map((row, idx) => (
            <div
              key={idx}
              className={`grid grid-cols-7 gap-4 px-6 py-3 text-sm ${
                idx % 2 === 0 ? "bg-gray-50" : "bg-white"
              }`}
            >
              <div>{row.symbol}</div>
              ...
              <div
                className={
                  row.pnl.startsWith("-") ? "text-red-500" : "text-emerald-500"
                }
              >
                {row.pnl}
              </div>
      ...
      <div className="bg-stone-200/50 h-8 flex justify-center items-center rounded-b-xl">
        <div className="w-6 h-6 flex items-center justify-center text-blue-900 text-xl">
          ▼
        </div>
      </div>
    </div>
